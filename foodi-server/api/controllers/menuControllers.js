...
const updateMenuItem = async (req, res) => {
    ...
    try {
        const updateMenu = await Menu.findByIdAndUpdate(id, { name, recipe, image, category, price }, { new: true, runValidators: true })
        if (!updateMenu) {
            res.status(500).json({message: "Menu Item not found"})
        }
